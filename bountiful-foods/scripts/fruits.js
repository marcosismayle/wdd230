document.addEventListener('DOMContentLoaded', () => {
  ...
  const createTable = (fruits) => {
    const table = document.createElement('table');

    const headerRow = document.createElement('tr');

    const headers = ['Name', 'ID', 'Genus', 'Family', 'Order', 'Carbohydrates', 'Protein', 'Fat', 'Calories', 'Sugar'];
    ...
    fruits.forEach(fruit => {
      ...
      idCell.textContent = fruit.id;
      dataRow.appendChild(idCell);

      const genusCell = document.createElement('td');
      genusCell.textContent = fruit.genus;
      dataRow.appendChild(genusCell);

      const familyCell = document.createElement('td');
      familyCell.textContent = fruit.family;
      dataRow.appendChild(familyCell);

      const orderCell = document.createElement('td');
      orderCell.textContent = fruit.order;
      dataRow.appendChild(orderCell);

      const carboCell = document.createElement('td');
      carboCell.textContent = fruit.nutritions.carbohydrates;
      dataRow.appendChild(carboCell);
  ...
  fruits.forEach((fruit) => {
    ...
    option.value = fruit.id; // Usar fruit.id como valor da opção
